                ...
                <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                    ...
                    @auth
                        @if (Auth::user()->isAdmin())
                            ...
                            <x-nav-link :href="route('reports.bookings')" :active="request()->routeIs('reports.bookings')">
                                {{ __('Laporan') }}
                            </x-nav-link>
                        @endif

                        @if (Auth::user()->isApprover(1))
                            <x-nav-link :href="route('approvals.level1.index')" :active="request()->routeIs('approvals.level1.*')">
                                {{ __('Persetujuan Level 1') }}
                            </x-nav-link>
                        @endif

                        @if (Auth::user()->isApprover(2))
                            <x-nav-link :href="route('approvals.level2.index')" :active="request()->routeIs('approvals.level2.*')">
                                {{ __('Persetujuan Level 2') }}
                            </x-nav-link>
                        @endif
        ...
        <div class="pt-2 pb-3 space-y-1">
            ...
            @auth
                @if (Auth::user()->isAdmin())
                    ...
                    <x-responsive-nav-link :href="route('reports.bookings')" :active="request()->routeIs('reports.bookings')">
                        {{ __('Laporan') }}
                    </x-responsive-nav-link>
                @endif

                @if (Auth::user()->isApprover(1))
                    <x-responsive-nav-link :href="route('approvals.level1.index')" :active="request()->routeIs('approvals.level1.*')">
                        {{ __('Persetujuan Level 1') }}
                    </x-responsive-nav-link>
                @endif

                @if (Auth::user()->isApprover(2))
                    <x-responsive-nav-link :href="route('approvals.level2.index')" :active="request()->routeIs('approvals.level2.*')">
                        {{ __('Persetujuan Level 2') }}
                    </x-responsive-nav-link>
                @endif
