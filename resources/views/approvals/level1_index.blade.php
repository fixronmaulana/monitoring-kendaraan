<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Persetujuan Pemesanan Level 1') }}
        </h2>
    </x-slot>

    <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
        <div class="p-6 text-gray-900">
            <h3 class="text-lg font-bold mb-4">Pemesanan Menunggu Persetujuan Anda (Level 1)</h3>
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200 rounded-lg overflow-hidden">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @forelse ($approvals as $approval)
                            <tr>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                    <form action="{{ route('approvals.level1.update', $approval) }}" method="POST" class="inline-block">
                                        @csrf
                                        @method('PUT')
                                    ...
                                    <form action="{{ route('approvals.level1.update', $approval) }}" method="POST" class="inline-block ml-2">
                                        @csrf
                                        @method('PUT')
                        ...
                        @empty
                            <tr>
                                <td colspan="6" class="px-6 py-4 whitespace-nowrap text-center text-gray-500">Tidak ada pemesanan menunggu persetujuan Level 1.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
