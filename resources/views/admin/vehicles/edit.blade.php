            ...
            <form action="{{ route('admin.vehicles.update', $vehicle) }}" method="POST">
                ...
                <div class="mb-4">
                    ...
                    <select id="ownership_type" name="ownership_type" class="block mt-1 w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm" required>
                        ...
                        <option value="Milik Perusahaan" {{ old('ownership_type', $vehicle->ownership_type) == 'Milik Perusahaan' ? 'selected' : '' }}>Milik Perusahaan</option>
                        <option value="Sewa" {{ old('ownership_type', $vehicle->ownership_type) == 'Sewa' ? 'selected' : '' }}>Sewa</option>
                    </select>
                ...
                <div class="mb-4">
                    ...
                    <textarea id="description" name="description" rows="3" class="block mt-1 w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm">{{ old('description', $vehicle->description) }}</textarea>
                    <x-input-error :messages="$errors->get('description')" class="mt-2" />
                </div>

                <div class="flex items-center justify-end mt-4">
                    <x-primary-button>
                        {{ __('Perbarui Kendaraan') }}
                    </x-primary-button>
                </div>
            </form>
        </div>
